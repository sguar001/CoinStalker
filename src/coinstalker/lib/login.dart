...
class _LoginPageState extends State<LoginPage> {
  ...
  bool validateAndSave() {
    ...
    if (form.validate()) {
      form.save();
      return true;
      print('Form is valid: $_email, password: $_password');
    } else {
      return false;
  ...
  void moveToLogin() {
    formKey.currentState.reset(); //Clears the form
    setState(() {
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
        appBar: new AppBar(
          title: new Text('Coin Stalker Login'),
        ),
        body: new Container(
  ...
  List<Widget> buildSubmitButtons() {
    if (_formType == FormType.login) {
      ...
      return [
        ...
        new FlatButton(child: new Text (
            'Have an account', style: new TextStyle(fontSize: 20.0)),
