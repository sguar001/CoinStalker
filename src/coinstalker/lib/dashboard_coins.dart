...
class _DashboardCoinsState extends State<DashboardCoins> {
  ...
  /*Widget _coinTile(BuildContext context, Coin coin) => Card(
        ...
        child: ExpansionTile(
          title: Row(
            children: [
              ...
              futureWidget(
                future:
                CryptoCompare().price(coin.symbol, _profile.displaySymbol),
                waitBuilder: emptyWaitBuilder,
                builder: (context, price) => Text('$price'),
              ),
              futureWidget<double>(
                future: CryptoCompare()
                    .price(coin.symbol, _profile.displaySymbol)
                    .then((price) => CryptoCompare()
                    .hourOhlcv(coin.symbol, _profile.displaySymbol,
                    limit: 24)
                    .then((data) => data.first.close)
                    .then((close) => (price.price - close) / close)),
                waitBuilder: emptyWaitBuilder,
                ...
                children: [
                  futureWidget(
                    future: CryptoCompare().minuteOhlcv(
                        coin.symbol, _profile.displaySymbol,
                        limit: 60),
